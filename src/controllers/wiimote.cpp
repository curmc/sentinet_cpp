...
/* KermitControlModule funcitons. I may want to remove the connect and loop
 * functions and just put their bodies inside here */
int
Wiimote::__start__()
{
  connect();
...
Wiimote::__quit__()
{
  ...
  // I'd rather use the destructor since I know that will absolutely be called
  // to handle deallocation
  return 1;
}

inline float
Wiimote::__get_lin__() const
{
  return linear;
}

inline float
Wiimote::__get_ang__() const
{
  return angular;
}

inline KERMIT_STATE
Wiimote::__get_state__() const
{
  return state;
}
...
int
Wiimote::loop(high_resolution_clock::time_point now)
{
  poll();
  control(now);
 ...
 */
int
Wiimote::control(const high_resolution_clock::time_point& now)
{
  ...
  const auto toggle_state = [&](KERMIT_STATE toggling, const unsigned& button) {
    if (IS_JUST_PRESSED(data[0], button)) {
      ...
      /* Strange bug continuously toggles buttons until another one is pressed.
       * Here's a fix */
