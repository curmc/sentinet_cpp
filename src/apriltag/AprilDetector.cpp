...
AprilDetector::AprilDetector(const int threads, bool show_stream_, const std::string& tag_type, bool verbose) 
  : Localizer(verbose), multithreaded(threads)
{
...
bool AprilDetector::async_start() {
  return false;
}
...
bool AprilDetector::loop() {
  ...
  if(video.show_stream) {
    for(int i = 0; i < zarray_size(detections); ++i) {
      ...
      putText(video.frame, text, Point(det->c[0] - textsize.width / 2,
...
void AprilDetector::destroy_video() {
  return;
}
