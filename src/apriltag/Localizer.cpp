...
using namespace addr::localizer;
...
Localizer::Localizer(bool verbose)
{
  ...
  network.camera_p.get_data = std::bind(&Localizer::get_pos, this);
...
bool
Localizer::localizer_start(const std::chrono::seconds period_)
{
...
void
Localizer::set_pos(float x, float y, float z, float theta)
{
...
std::string
Localizer::get_pos(void)
{
  std::lock_guard<std::mutex> lock(guard);
  pos_localizer_to_wire(&message.loc);
  return std::string(reinterpret_cast<char const*>(message.loc.buff.data),
                     message.size);
}

void
Localizer::print_state()
{
  std::lock_guard<std::mutex> lock(guard);
  std::cout << "x: " << message.loc.x << " y: " << message.loc.y
            << " z: " << message.loc.z << " theta: " << message.loc.theta
            << std::endl;
}
