...
int SCPPSession::parse_command_line(int argc, const char** argv) {
  ...
  for(i = 0; i < args.size(); ++i) {
    switch(args[i]) {
      case("-f"): {
        ...
        // Set number of publisher proxies if session.num_pub doesn't already exist, otherwise 1
        sess.num_pub = (!sess.num_pub && ++i < args.size()) ? strtol(args[i]) : 0;

        // Either one of these is empty means invalid argument
        if(!sess.starting_pub_port_b)
          std::cout<<"-f expected an integer port followed by number of ports\n";
        break;
      }
      ...
      case("-b"): {
        ...
        // Set number of publishers if session.num_pub doesn't already exist, otherwise 1
        sess.num_pub = (!sess.num_pub && ++i < args.size()) ? strtol(args[i]) : 0;

        // Either one of these is empty means invalid argument
        if(!sess.starting_pub_port_b)
          std::cout<<"-b expected an integer port followed by number of ports\n";
        break;
      }
    }
