...
void ProxyInterface::start(const std::chrono::microseconds t) {
  ...
  while(!paused && running && exit_signal.wait_for(std::chrono::milliseconds(0)) == std::future_status::timeout) {
...
bool ProxyInterface::pause() {
  ...
  paused = true;
  while(running && exit_signal.wait_for(std::chrono::milliseconds(0)) == std::future_status::timeout && paused) {
...
bool ProxyInterface::register_signal(const int32_t signal_val, std::function<int(void)> func) {
  ...
  if(found != signal_table.end()) {
    LOG_ERROR("Proxy of id %s failed to register signal %d, signal already exists", id.c_str(), signal_val);
    return false;
  }

  // Return the emplace function emplace status
  if(!signal_table.emplace(signal_val, func).second){
    LOG_ERROR("Proxy of id %s failed to emplace signal for some reason, it doesn't exist in the map though", id.c_str());
    return false;
  }
  return true;
}
