...
int signal_teensy36_header(int fd, uint8_t header) {
...
static int send_teensy36_linear(int fd, int16_t linear) {
...
static int send_teensy36_angular(int fd, int16_t angular) {
...
int send_teensy36_cmd_vel(int fd, int16_t linear, int16_t angular) {
...
int teensy36_recieve_ack(int fd) {
  printf("Reading\n");
...
int teensy36_send_command(int fd, uint8_t command) {
...
int xhci_hcd_write_teensy36(int fd, const uint8_t* buffer, size_t buffer_len) {
  printf("here\n");
...
int xhci_hcd_read_teensy36(int fd, uint8_t* destination, size_t num_bytes) {
  return read(fd, (void*)destination, num_bytes); 
}

int teensy36_fd_is_valid(int fd) {
  return fcntl(fd, F_GETFD) != -1 || errno != EBADF;
}

int xhci_hcd_teensy36_opt(const char* port) { 
  int fd;
  ...
  if(!isatty(fd)) {
    close(fd);
    fprintf(stderr, "Error, port is not a tty: %d : %s\n", errno, strerror(errno));
  }
  ...
  speed_t brate = B9600;
  if(cfsetispeed(&teensy_opt, brate) < 0 || cfsetospeed(&teensy_opt, brate) < 0) {
    close(fd);
    fprintf(stderr, "Speed error: %d : %s\n", errno, strerror(errno));
    return -1;
  }

  teensy_opt.c_cflag = teensy_opt.c_cflag & ~PARENB & ~CSTOPB & ~CSIZE | CS8 & ~CRTSCTS | CREAD | CLOCAL;
  teensy_opt.c_iflag = teensy_opt.c_iflag & ~IXON & ~IXOFF & ~IXANY & ICANON & ECHO & ~ECHOE & ~ISIG;
  ...
  if(ioctl(fd, TIOCEXCL, NULL) < 0) {
    close(fd);
    fprintf(stderr, "Cannot grant exclusive access: %d : %s\n", errno, strerror(errno));
    return -1;
  }
