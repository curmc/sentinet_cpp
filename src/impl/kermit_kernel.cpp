  ...
  while(i < argc) {
    if(!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help") || argc == 1) {
      std::cout<<
        ...
        "   --verbose     Show linear and angular values of kermit consistantly\n\n"

        "   --ip          If Debug is on, this does nothing. Otherwise, specify\n"
        "                 the ip address of the teensy (default 192.168.0.2)\n\n"

        "   --port        If Debug is on, this does nothing. Otherwise, specify\n"
        "                 the port of the teensy (default 80)\n\n"

        "   --interface   If Debug is on, this does nothing. Otherwise, specify\n"
