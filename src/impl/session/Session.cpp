...
int create_server_proxy(const std::string& id, const std::string& frontend, const std::string& backend) {
  std::promise<void> futureobj;
  servers.push_back(new scpp::proxies::ZMQServerProxy(id, std::move(futureobj), frontend, backend));
  return 1;
}
